-- Harden role-aware RLS and add transactional workflow RPCs used by Edge Functions

create or replace function public.recalculate_inventory_balance(p_tenant_id uuid, p_merchandise_id uuid, p_location_id uuid)
returns void
language plpgsql
security definer
as $$
...
begin
  select
    coalesce(sum(case when movement_type in ('in','adjustment','transfer') then quantity when movement_type='out' then -quantity else 0 end), 0),
    coalesce(sum(case when movement_type='reservation' then quantity when movement_type='release' then -quantity else 0 end), 0)
  into v_on_hand, v_reserved
  from public.inventory_movements
  where tenant_id = p_tenant_id and merchandise_id = p_merchandise_id and location_id = p_location_id;

  insert into public.inventory_balances (tenant_id, merchandise_id, location_id, quantity_on_hand, quantity_reserved, quantity_available, updated_at)
  values (p_tenant_id, p_merchandise_id, p_location_id, v_on_hand, v_reserved, (v_on_hand - v_reserved), now())
  on conflict (tenant_id, merchandise_id, location_id)
  do update set
...
create or replace function public.api_createTenant(payload jsonb)
returns jsonb
language plpgsql
security definer
as $$
...
begin
  ...
  values (payload->>'name', payload->>'slug', coalesce(payload->>'status', 'active'))
...
-- RLS hardening for client visibility

drop policy if exists people_tenant_read on public.people;
create policy people_tenant_read on public.people
for select using (
  tenant_id = public.current_tenant_id()
  and (
    public.is_admin() or public.is_member() or
    (public.is_client() and id = public.current_person_id())
  )
);

drop policy if exists sales_orders_tenant_read on public.sales_orders;
create policy sales_orders_tenant_read on public.sales_orders
for select using (
  tenant_id = public.current_tenant_id()
  and (
    public.is_admin() or public.is_member() or
    (public.is_client() and customer_person_id = public.current_person_id())
  )
);

drop policy if exists deliveries_tenant_read on public.deliveries;
create policy deliveries_tenant_read on public.deliveries
for select using (
  tenant_id = public.current_tenant_id()
  and (
    public.is_admin() or public.is_member() or
    (
      public.is_client() and (
        recipient_person_id = public.current_person_id() or
        order_id in (select so.id from public.sales_orders so where so.customer_person_id = public.current_person_id())
      )
    )
  )
);
...
drop policy if exists digital_files_read on public.digital_files;
create policy digital_files_read on public.digital_files
for select using (
  exists (
    select 1
    from public.objects o
    where o.id = digital_files.object_id
      and o.tenant_id = public.current_tenant_id()
      and (
        public.is_admin() or public.is_member() or
        (public.is_client() and o.owner_type = 'person' and o.owner_person_id = public.current_person_id())
      )
  )
);

alter table public.delivery_events enable row level security;
drop policy if exists delivery_events_tenant_read on public.delivery_events;
create policy delivery_events_tenant_read on public.delivery_events
for select using (
  exists (
    ...
    where d.id = delivery_events.delivery_id
      ...
      and (
        public.is_admin() or public.is_member() or
        (public.is_client() and (d.recipient_person_id = public.current_person_id() or d.order_id in (select so.id from public.sales_orders so where so.customer_person_id = public.current_person_id())))
      )
  )
);
